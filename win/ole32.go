...
var (
	...
	// Functions
	cLSIDFromString       *windows.LazyProc
	...
	stringFromGUID2       *windows.LazyProc
)
...
func init() {
	...
	// Functions
	cLSIDFromString = libole32.NewProc("CLSIDFromString")
	...
	stringFromGUID2 = libole32.NewProc("StringFromGUID2")
}

func CLSIDFromString(lpsz *uint16, pclsid *windows.GUID) (ret error) {
	r0, _, _ := syscall.Syscall(cLSIDFromString.Addr(), 2, uintptr(unsafe.Pointer(lpsz)), uintptr(unsafe.Pointer(pclsid)), 0)
	if r0 != 0 {
...
func StringFromGUID2(rguid *windows.GUID) (string, bool) {
	nSize := 0
	lpBuffer := []uint16{0}
	ret, _, _ := syscall.Syscall(stringFromGUID2.Addr(), 3,
		uintptr(unsafe.Pointer(rguid)),
		uintptr(unsafe.Pointer(&lpBuffer[0])),
		uintptr(nSize))

	return syscall.UTF16ToString(lpBuffer), ret != 0
}
