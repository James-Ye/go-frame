...
var (
	...
	registerDeviceNotificationW  *windows.LazyProc
	...
	unregisterDeviceNotification *windows.LazyProc
	updateWindow                 *windows.LazyProc
	waitForInputIdle             *windows.LazyProc
...
func init() {
	...
	registerDeviceNotificationW = libuser32.NewProc("RegisterDeviceNotificationW")
	...
	unregisterDeviceNotification = libuser32.NewProc("UnregisterDeviceNotification")
	updateWindow = libuser32.NewProc("UpdateWindow")
	waitForInputIdle = libuser32.NewProc("WaitForInputIdle")
...
func GetClipboardData(uFormat uint32) windows.Handle {
...
func GetSystemMetricsForDpi(nIndex int32, dpi uint32) (int32, error) {
...
func LoadImage(hinst HINSTANCE, lpszName *uint16, uType uint32, cxDesired, cyDesired int32, fuLoad uint32) windows.Handle {
...
func RegisterDeviceNotification(hRecipient windows.Handle, NotificationFilter uintptr, Flags uint32) uintptr {
	ret, _, _ := syscall.Syscall(registerDeviceNotificationW.Addr(), 3,
		uintptr(hRecipient),
		NotificationFilter,
		uintptr(Flags))
...
func SetClipboardData(uFormat uint32, hMem windows.Handle) windows.Handle {
...
func UnregisterDeviceNotification(Handle uintptr) bool {
	ret, _, _ := syscall.Syscall(unregisterDeviceNotification.Addr(), 1, Handle, 0, 0)
	return ret != 0
}
...
func WaitForInputIdle(hProcess windows.Handle, Milliseconds uint32) uint32 {
	ret, _, _ := syscall.Syscall(waitForInputIdle.Addr(), 2,
		uintptr(hProcess),
...
func PostThreadMessage(idThread uint32, Msg uint, wParam WPARAM, lParam LPARAM) bool {
	ret, _, _ := syscall.Syscall6(postThreadMessageW.Addr(), 4,
		uintptr(idThread),
		uintptr(Msg),
